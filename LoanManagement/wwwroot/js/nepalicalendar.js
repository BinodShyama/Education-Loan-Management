...
(function ($) {
    ...
    $.extend(NepaliCalendar, {
        ...
        getEnglishDate: function (npYear, npMonth, npDay) {
            var days = 0;
            var initday = 0;
            npYear = parseInt(npYear);
            npMonth = parseInt(npMonth);
            npDay = parseInt(npDay)
            var dt = new Date();
            switch (npMonth) {
                case 1: case 2: case 3: case 4: case 5: case 6: case 7: case 8:
                    ...
                    days += npDay > calendarData.nepalicaledarData[npYear - 57][i] ? calendarData.nepalicaledarData[npYear - 57][i] : npDay;
                ...
                case 9:
                    initday = calendarData.nepalicaledarData[npYear - 57][1];
                    npDay = npDay > calendarData.nepalicaledarData[npYear - 57][14] ? calendarData.nepalicaledarData[npYear - 57][14] : npDay;
                    if (npDay < initday) {
                        for (i = 3; i < 14; i++) {
                ...
                case 10: case 11: case 12:
                    ...
                    days += calendarData.nepalicaledarData[npYear - 56][2] - calendarData.nepalicaledarData[npYear - 56][1];
                    days += npDay > calendarData.nepalicaledarData[npYear - 56][i] ? calendarData.nepalicaledarData[npYear - 56][i] : npDay;//npDay;
                    dt = new Date(npYear - 56, 0, 1);
                    dt.setDate(dt.getDate() + days);
                    break;
            }
...
(function ($) {
    var nepaliDate = NepaliCalendar.getBSDate((new Date()).getFullYear(), (new Date()).getMonth(), (new Date()).getDate())
    $('select.dob-year').append('<option value="">Year...</option>');
    for (let i = 2005; i < nepaliDate.npYear - 10; i++) {
        $('select.dob-year').append('<option value="' + i + '">' + i + '</option>');
    }
    $('select.year').append('<option value="">Year...</option>');
    for (let i = 2005; i < nepaliDate.npYear; i++) {
        $('select.year').append('<option value="' + i + '">' + i + '</option>');
    }
    $('select.month').append(`'<option value="">Month...</option>
    ...
    $('select.day').append('<option value="">Day...</option>');
    for (let i = 1; i < 33; i++) {
